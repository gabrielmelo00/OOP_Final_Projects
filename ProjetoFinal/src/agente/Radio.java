...
public class Radio extends Agente {
	
	private Image imgRadio;
	private Comodo meuComodo;
	private int contadorTempoCelula;

	public Radio(int i, int j, int escala, Comodo meuComodo, int ciclos) {
		super(i, j, escala, 'V', ciclos);
		this.meuComodo = meuComodo;
		contadorTempoCelula = 0;
		imgRadio = new ImageIcon(Carregador.Imagens.get(Carregador.RADIO).getImage().getScaledInstance(escala,escala, 1)).getImage();
	}
