...
public class Louca extends Agente {

	private Image imgLouca;
	private Comodo meuComodo;
	private int contadorTempoCelula;
	private int estado;
	

	public Louca(int i, int j, int escala, Comodo meuComodo, int ciclos) {
		super(i, j, escala, 'V', ciclos);
		this.meuComodo = meuComodo;
		contadorTempoCelula = 0;
		estado = 0;
		imgLouca = new ImageIcon(Carregador.Imagens.get(Carregador.LOUCA).getImage().getScaledInstance(escala,escala, 1)).getImage();
